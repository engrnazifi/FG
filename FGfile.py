...
def flutterwave_webhook():
    try:
        ...
        if not signature:
            bot.send_message(ADMIN_ID, "❌ No signature")
        ...
        if status not in ("successful", "success"):
            bot.send_message(ADMIN_ID, f"⚠️ Ignored status: {status}")
            return "Ignored", 200
...
""")
        ...
        if not order_id:
            bot.send_message(ADMIN_ID, "❌ No order_id extracted")
        ...
        # ================= WALLET =================
        if not row:
            bot.send_message(ADMIN_ID, "🔄 Checking wallet...")
            ...
            if not dep:
                bot.send_message(ADMIN_ID, "❌ Order not found anywhere")
...
""")
            ...
            if paid_amount < expected_amount or currency != "NGN":
                bot.send_message(ADMIN_ID, "❌ Underpaid wallet")
                return "Wrong payment", 200

            wallet_cur.execute("""
                UPDATE wallet_deposits
                SET status='success', paystack_ref=%s, paid_at=NOW()
                WHERE id=%s
            ...
            bot.send_message(ADMIN_ID, "✅ Wallet success")
            return "OK", 200
...
""")
        ...
        # ✅ FIXED CONDITION
        if paid_amount < expected_amount or currency != "NGN":
            bot.send_message(ADMIN_ID, "❌ Underpaid order")
            return "Wrong payment", 200
        ...
        bot.send_message(ADMIN_ID, "✅ Order marked paid")

        # ================= FILM =================
        if order_type == "film":
            bot.send_message(ADMIN_ID, "🎬 Film order processing")
...
""",
            ...
            bot.send_message(ADMIN_ID, "✅ Film delivered")
        ...
        # ================= VIP =================
            ...
            bot.send_message(ADMIN_ID, "💎 VIP processing")
